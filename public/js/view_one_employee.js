...
const deleteEmloyee = (id) => {
    if(confirm("Are You Realy Want to Delete Emoloyee? Deleting an employee also delete its salary detail."))
        fetch(`/admin/delete-employee/${id}`, {
            method: "DELETE",
            headers: { 
        ...
        .then((res) => {
            if(!res.error){
                window.location.replace("/admin/view-all-employees")
            }
            else if(res.error){
